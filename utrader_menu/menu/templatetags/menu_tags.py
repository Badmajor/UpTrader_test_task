...
def build_menu_structure(menu, named_url):
    menu_structure = dict()
    ...
    for item in items:
        if item.named_url == named_url:
            ...
            if hasattr(item.parent, 'named_url'):
                menu_structure.update(build_menu_structure(menu, item.parent.named_url))
    menu_structure[None] = [item for item in items if not item.parent_id]
    return menu_structure


def render_menu_html(menu_structure, current_path, parent=None):
    html = '<ul>'
    for item in menu_structure.get(parent, []):
        active = 'active' if current_path == item.named_url else ''
        html += f'<li class="{active}"><a href="{item.named_url}">{item.name}</a>'
        if item.id in menu_structure:
            html += render_menu_html(menu_structure, current_path, item.id)
        html += '</li>'
    html += '</ul>'
    return html
